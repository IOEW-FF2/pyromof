...
def refine_received_electricity_price_data (raw_data):
...
electricity_price_data_germany_cent_per_kWh = refine_received_electricity_price_data(electricity_price_data_germany_euro_per_MWh.copy())
...
print(f"first 10 columns of the pyrolysis electricity to grid data: \n{pyrolysis_electricity_to_grid.head(10)}")
...
feed_in_payment_series = feed_in_payment(electricity_price_data_germany_cent_per_kWh.copy(), base_value, lower_treshold)


# create data for government payment to renewable energy producers (cent/kWh)

government_payment_share = government_payment(electricity_price_data_germany_cent_per_kWh.copy(), feed_in_payment_series.copy(), base_value)


# calculate received payment for electricity fed into the grid (euro)

received_payment_series = 1/100 * multiply(feed_in_payment_series.copy(), pyrolysis_electricity_to_grid.copy())


# calculate payment share of government for electricity from the pyrolysis plant (euro) 

government_payment_for_pyrolysis_plant = 1/100 * multiply(government_payment_share.copy(), pyrolysis_electricity_to_grid.copy())


# calculate payment share of electricity market for electricity from the pyrolysis plant

electricity_market_payment_for_pyrolysis_plant = received_payment_series.copy() - government_payment_for_pyrolysis_plant.copy()
...
print(f"government payment + electricity market payment: {sum_government_payment.round(1) + sum_electricity_market_payment.round(1)} euro")
