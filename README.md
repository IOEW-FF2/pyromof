...
In order to run the model you need a solver. By default, the code expects the solver cbc. If you want to use a different one, you have to adjust the code in pyromof/optimize.py. If you use wondows, the file "cbc.exe" should be placed in the same folder as this readme-file.
The input data are not provided within this repository. A discription on how to design the input data and how to run the model is provided in docs/usage.md.
